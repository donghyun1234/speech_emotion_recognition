...
class TraditionalCNN(nn.Module):
    def __init__(self, in_chennel=3, out_chennel=128, conv_kernel=(3,3), conv_stride=(1,1), conv_padding=(0,0), p=0.5, pool_kernel=(2,4), pool_stride=(2,4)):
...
class acrnn(nn.Module):
    def __init__(self, drop=0):
        ...
        self.cnn1 = TraditionalCNN(in_chennel=3, out_chennel=128, conv_kernel=(3,5), conv_stride=(1,1), conv_padding=(1,2), p=drop, pool_kernel=(2,2), pool_stride=(2,2))
        self.cnn2 = TraditionalCNN(in_chennel=128, out_chennel=256, conv_kernel=(3,5), conv_stride=(1,1), conv_padding=(1,2), p=drop, pool_kernel=(1,1), pool_stride=(1,1))
        # self.cnn3 = TraditionalCNN(in_chennel=256, out_chennel=256, conv_kernel=(3,5), conv_stride=(1,1), conv_padding=(1,2), p=drop, pool_kernel=(1,1), pool_stride=(1,1))
        # self.cnn4 = TraditionalCNN(in_chennel=256, out_chennel=256, conv_kernel=(3,5), conv_stride=(1,1), conv_padding=(1,2), p=drop, pool_kernel=(1,1), pool_stride=(1,1))
        # self.cnn5 = TraditionalCNN(in_chennel=256, out_chennel=256, conv_kernel=(3,5), conv_stride=(1,1), conv_padding=(1,2), p=drop, pool_kernel=(1,1), pool_stride=(1,1))
        # self.cnn6 = TraditionalCNN(in_chennel=256, out_chennel=256, conv_kernel=(3,5), conv_stride=(1,1), conv_padding=(1,2), p=drop, pool_kernel=(1,1), pool_stride=(1,1))

        self.linear1 = nn.Linear(in_features=20*256, out_features=768)

        self.bilstm = nn.LSTM(input_size=768, hidden_size=128, num_layers=1, bidirectional=True,dropout=drop)
    ...
    def forward(self,x):
        ...
        out, (hn, cn) = self.bilstm(x.permute(1,0,2))

        # Attention
        attened, alpha = self.attention(out,True)
        x = self.linear2(attened)
        x = F.leaky_relu(x,0.01)
...
def train(x, y, model, criterion, optimizer, device):
    ...
    return loss, y_pred.cpu().detach().numpy(), train_acc
...
def main(data_path, model_name, pred_name, prob, epoch):
    num_epoch = epoch
    num_classes = 4
    batch_size = 40
    ...
    ##########define model###########
    model = acrnn(prob).to(device)
#    wandb.init(project='acrnn',name=str(prob))
#    wandb.watch(model, log='all')

    ##########tarin model###########
    criterion = [nn.CrossEntropyLoss().to(device), nn.MSELoss().to(device)]
    optimizer = optim.Adam(model.parameters(), lr=0.00001, betas = (0.5, 0.999),  weight_decay = 1e-5)
    # lambda1 = lambda epoch: (min_learning_rate + (max_learning_rate - min_learning_rate) * math.exp(-epoch / decay_speed))
    ...
    for i in range(num_epoch):
      ...
      #      wandb.log({'loss':tcost})
        ...
        if i % 5 == 0:
            ...
            print("*****************************************************************")
            print("Epoch: %05d" % (i + 1))
            print("Training cost: %2.3g" % tcost.item())
...
def evaluate(data_path, model_name, pred_name,prob):
    num_epoch = 5000
    num_classes = 4
    batch_size = 40
    learning_rate = 0.00001
...
if __name__=='__main__':
  ...
  #  iterate = [5000,5500,6000,6500,7000,7500,8000,8500,9000,9500,10000]
    iterate = [4000,4000,4000,4000,4000]

    model_name, pred_name = make_dir('param' + str(13))

    for i in range(len(data_path)):
        main(data_path[i], model_name[i], pred_name[i], prob=0, epoch=3000)
        torch.cuda.empty_cache()
        evaluate(data_path[i], model_name[i], pred_name[i], prob=0)
        torch.cuda.empty_cache()
