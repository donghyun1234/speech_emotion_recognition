...
class TraditionalCNN(nn.Module):
    def __init__(self, in_chennel=3, out_chennel=128, conv_kernel=(3,3), conv_stride=(1,1),conv_padding=(1,1), p=0.0, pool_kernel=(2,4), pool_stride=(2,4)):
...
class UFLB(nn.Module):
    def __init__(self, in_chennel=128, out_chennel=128, conv_kernel=(3,3), conv_stride=(1,1),dilate_kernel=(2,2),pad=2, activate=True):
        super(UFLB, self).__init__()

        if activate == True:
            self.uflb = nn.Sequential(
                nn.Conv2d(in_chennel, out_chennel, kernel_size=conv_kernel, stride=conv_stride, dilation=dilate_kernel, padding=pad),
                nn.BatchNorm2d(out_chennel),
                nn.LeakyReLU(0.01)
            )
        else:
            self.uflb = nn.Sequential(
...
class adrnn(nn.Module):
    def __init__(self,input_size):
        ...
        self.cnn = TraditionalCNN()
        self.uflb1 = UFLB(in_chennel=128, out_chennel=128)
        self.uflb2 = UFLB(in_chennel=128, out_chennel=128)
        self.uflb3 = UFLB(in_chennel=128, out_chennel=256)

        self.skipped = UFLB( in_chennel=128, out_chennel=256, activate=False)
...
def main(data_path, model_name, pred_name):
    ...
    ##########define model###########
    model = adrnn(train_data.shape).to(device)
